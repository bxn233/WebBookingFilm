...
function SampleNextArrow(props) {
    ...
    return (
          <div
            ...
            style={{ ...style, display: "block", position: 'absolute', right: '100px', top: '45%' }}
            onClick={onClick}
          />
    );
}
...
function SamplePrevArrow(props) {
    ...
    return (
        <div
            className={`${className} ${styleCarousel['slick-prev']}`}
            style={{ ...style, display: "block", position: 'absolute', left: '50px', top: '45%', zIndex: 10 }}
            onClick={onClick}
        />
    );
}
...
export default function NewHomeCarousel() {
    ...
    const renderBanner = () => {
        return bannerList.map((item, index) => (
            <div key={index}>
                <div className='h-48 md:h-64 lg:h-80 xl:h-96' style={{ ...settings, background: `url(${item.hinhAnh}) 100% center no-repeat`, width: '100%', backgroundSize: 'cover' }}>
